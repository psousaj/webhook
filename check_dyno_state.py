...
APP = "WEBHOOK"
WEBHOOK_API = (
    load_env("WEBHOOK_API_LOCAL") if is_local_machine() else load_env("WEBHOOK_API")
)
...
def check_state():
    with Client(base_url=f"{HEROKU_API}", headers=client_request_header) as client:
            ...
            while True:
                ...
                if dyno_state_request.status_code == 200:
                    ...
                    if state in NOT_ALLOWED_DYNO_STATES:
                        ...
                        logger.info(
                            f"{dyno} Dyno. There's something wrong here, state:{state}"
                        )
                ...
                if retries >= max_retries:
                    ...
                    retries += 1
                    logger.info(
                        f"Retrying {dyno} state check ({retries}/{max_retries})"
                    )
        ...
        logger.info(
            f"{APP}: Todos os dynos estão rodando bem! Checked-In: {currently_datetime()}"
        )
        return
